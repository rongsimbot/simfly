#!/usr/bin/env python3
"""
train_rl_torque_gpu.py — GPU-Accelerated RL Torque Decoder Training.

Same as train_rl_torque.py but uses PyTorch GPU for policy/value networks.
Connectome pipeline + MuJoCo still run CPU-side (rollout bottleneck).
PPO update runs entirely on CUDA.
...
# ── Imports ─────────────────────────────────────────────────────────────
from rl_bridge import (           # Original: config, modulation, env, pipeline
    RLConfig, apply_modulation, SimFlyRLEnv,
)
from rl_bridge_gpu import (       # GPU: policy + PPO trainer
    ConnectomeModulationPolicy, train_ppo,
)
...
def run_rl_policy_baseline(pipeline: SimFlyRLPipeline, policy,
    ...
    for step in range(n_eval_steps):
        ...
        squashed, _, _, raw = policy.act(obs)
...
def plot_learning_curve(history: List[Dict], output_path: str):
    ...
    try:
        ...
        ax1.set_title('PPO Training: SimFly Torque Calibration (GPU)')
...
def main():
    ...
    if args.output:
        ...
        output_dir = os.path.join(RL_TRAINING_DIR, "rl_training_output_gpu")
    ...
    # ── Train RL Policy (GPU PPO) ─────────────────────────────────────
    print("\n[3/4] Training RL torque calibration policy (GPU PPO)...", flush=True)
    ...
    print("  Starting GPU PPO iterations...", flush=True)
