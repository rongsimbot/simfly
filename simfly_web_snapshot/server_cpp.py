...
class SimFlyLoop:
    def __init__(self, engine: CppEngine, sensor_idx, bridge, decoder, loader,
        ...
        # ── Phase 16: Per-Actuator RL Control ──
        # External torque overrides from RL agents (joint_name → torque)
        self._external_torques: Dict[str, float] = {}
        # Joint state tracking for RL state reads
        self._joint_states: Dict[str, Dict[str, float]] = {}
        # Per-joint qpos address mapping
    ...
    def step(self):
        ...
        if self.decoder:
            self.decoder.accumulate_weighted(mn_activations if mn_activations else {})
        joint_commands = self.decoder.decode() if self.decoder else {}

        self.data.ctrl[:] = 0.0
        nz = 0
        target_set = self.target_joints  # Phase 15: per-actuator isolation
        # ── Phase 16: External torque overrides from RL agents ──
        # RL agents post torque via /api/actuator/torque — these override decoder
        for jname, ext_torque in list(self._external_torques.items()):
            joint_commands[jname] = ext_torque
        self._external_torques.clear()  # consume once per step
        ...
        # ── Phase 16: Track per-joint state for RL agent reads ──
        for jname, qpos_idx in self._joint_qpos.items():
            self._joint_states[jname] = {
                'angle': float(self.data.qpos[qpos_idx]) if qpos_idx < len(self.data.qpos) else 0.0,
                'velocity': float(self.data.qvel[qpos_idx]) if qpos_idx < len(self.data.qvel) else 0.0,
            }
...
class SimFlyServer:
    ...
    def initialize(self, num_neurons=200, global_gain=0.002, tau_decay=50.0, target_joint=None):
        try:
            print(f"\n{'='*60}")
            ...
            for i in range(self.model.nu):
                ...
                a_name = name or f"act_{i}"
                self.act_map[a_name] = i
                # Phase 16: Map actuator → joint qpos address for RL state reads
                joint_id = self.model.actuator_trnid[i][0]
                qpos_adr = self.model.jnt_qposadr[joint_id]
                self.act_to_qpos[a_name] = qpos_adr
            ...
            # Phase 16: Pass qpos mapping to loop for joint state tracking
            self.loop._joint_qpos = dict(self.act_to_qpos)

            # Phase 15/16: Per-Actuator Connectome Control
            if target_joint:
                joints = [j.strip() for j in target_joint.split(",")]
                self.loop.target_joints = set(joints)
                print(f"  PHASE16: Isolated actuator mode - target joints: {joints}")
                print(f"  All other {self.model.nu - len(joints)} actuators = ZERO torque (neutral position)")
...
# ── Phase 16: Per-Actuator RL API Endpoints ──

@app.route('/api/actuator/torque', methods=['POST'])
def api_actuator_torque():
    """Apply torque to a specific actuator from RL agent."""
    if not sim_server._initialized or not sim_server.loop:
        return jsonify({'error': 'simulation not initialized'}), 503
    try:
        data = request.get_json(force=True)
        ...
        torque = float(np.clip(torque, -1.0, 1.0))
        sim_server.loop._external_torques[joint] = torque
        return jsonify({'success': True, 'joint': joint, 'torque': torque})
    except Exception as e:
        return jsonify({'error': str(e)}), 400

@app.route('/api/actuator/state/<joint>')
def api_actuator_state(joint):
    """
    Return per-actuator state for RL agent:
    - DN firing state, MN activation, joint angle, joint velocity
    """
    if not sim_server._initialized or not sim_server.loop:
        return jsonify({'error': 'simulation not initialized'}), 503
    ...
    # ── DN firing state ──
    dn_fired = 0
    dn_total = 953  # total matched DNs in bridge
    if hasattr(loop, 'bridge') and hasattr(loop.bridge, 'last_fired_dns'):
        dn_fired = len(loop.bridge.last_fired_dns)

    # ── MN activation ──
    mn_active = 0
    mn_mean = 0.0
    mn_total = 737  # total MNs in bridge
    if hasattr(loop, '_firing_history') and loop._firing_history:
        last = loop._firing_history[-1]
        mns_raw = last.get('mns', 0)
        # mns field is a list of MN IDs (up to 50), not a count
        if isinstance(mns_raw, list):
            mn_active = len(mns_raw)
        else:
            mn_active = int(mns_raw) if mns_raw else 0

    # Get total MN activation count from bridge report
    latest = sim_server.latest_metrics
    mn_active = latest.get('mns_activated', mn_active)

    # ── Joint state ──
    joint_state = loop._joint_states.get(joint, {'angle': 0.0, 'velocity': 0.0})
    joint_angle = joint_state.get('angle', 0.0)
    joint_velocity = joint_state.get('velocity', 0.0)

    return jsonify({
        'joint': joint,
        'dn_fired': dn_fired,
        'dn_total': max(dn_total, 1),
        'mn_active': mn_active,
        'mn_mean_activation': mn_mean,
...
def main():
    ...
    parser.add_argument('--target-joint', type=str, default=None,
                        help='Phase 16: isolate joints (comma-separated, e.g. coxa_T1_left,coxa_T1_right). All other joints get zero torque.')
    ...
    ok = sim_server.initialize(num_neurons=args.neurons, global_gain=args.global_gain, tau_decay=args.tau_decay, target_joint=args.target_joint)
    if not ok:
