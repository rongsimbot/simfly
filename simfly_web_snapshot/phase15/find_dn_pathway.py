...
# Build a grep pattern: "mn_id": <any of these>
...
dn_info = {}
for line in lines:
    m_dn = re.search(r'"manc_dn_id": (\d+)', line)
    m_in = re.search(r'"in_id": (\d+)', line)
    m_mn = re.search(r'"mn_id": (\d+)', line)
    m_dnt = re.search(r'"dn_type": "([^"]+)"', line)
    m_fw = re.search(r'"flywire_dn_id": "([^"]+)"', line)
...
print(f"Missing: {sorted(coxa_mn_ids - mn_matched) if coxa_mn_ids - mn_matched else 'none'}")
